...
static VkVertexInputAttributeDescription *getAttributeDescriptions(VkVertexInputAttributeDescription destination[2])
{
        ...
        memcpy(destination,attributeDescriptions,sizeof(attributeDescriptions)*2);
	return destination;
}
...
void createGraphicsPipeline(struct application *app)
{
	...
	vertexInputInfo.vertexAttributeDescriptionCount = (uint32_t)sizeof(attributeDescriptions)*2;/*magic number two as it's an array of 2*/
