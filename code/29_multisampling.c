...
void createColorResources(struct application *app);
...
void createImage(struct application *app,uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage *image, VkDeviceMemory *imageMemory);
VkImageView createImageView(struct application *app,VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels);
...
void transitionImageLayout(struct application *app,VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels);
...
void generateMipmaps(struct application *app, VkImage image,VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels);
...
void createImageViews(struct application *app) {
	...
	for (i = 0; i < app->imageCount; i++) {
		app->swapChainImageViews[i] = createImageView(app,app->swapChainImages[i],app->swapChainImageFormat,VK_IMAGE_ASPECT_COLOR_BIT, 1);
	}
}
void createRenderPass(struct application *app)
{
	...
	colorAttachment.samples = app->msaaSamples;
	...
	depthAttachment.samples = app->msaaSamples;
	...
	memset(&colorAttachmentResolve,0,sizeof(colorAttachmentResolve));
	colorAttachmentResolve.format = app->swapChainImageFormat;
	...
	memset(&colorAttachmentResolveRef,0,sizeof(colorAttachmentResolveRef));
...
VkSampleCountFlagBits getMaxUsableSampleCount(struct application *app)
{
...
void createTextureImageView(struct application *app)
{
	app->textureImageView = createImageView(app,app->textureImage,VK_FORMAT_R8G8B8A8_SRGB,VK_IMAGE_ASPECT_COLOR_BIT, 1);
}
void createTextureSampler(struct application *app)
{
	...
	samplerInfo.maxLod = app->mipLevels*1.0;
...
void createTextureImage(struct application *app)
{
	...
	app->mipLevels = (uint32_t)(floor(log2(fmax(texWidth, texHeight)))) + 1;
	...
	createImage(app,texWidth, texHeight, app->mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT| VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &app->textureImage, &app->textureImageMemory);

	transitionImageLayout(app,app->textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, app->mipLevels);
	...
	generateMipmaps(app, app->textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth, texHeight, app->mipLevels);
}
...
void generateMipmaps(struct application *app,VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
	...
	int32_t mipHeight = texHeight;
        ...
        for (uint32_t i = 1; i < app->mipLevels; i++) {
...
void createColorResources(struct application *app)
{
	VkFormat colorFormat = app->swapChainImageFormat;

	createImage(app,app->swapChainExtent.width, app->swapChainExtent.height, 1, app->msaaSamples, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &app->colorImage, &app->colorImageMemory);
	app->colorImageView = createImageView(app,app->colorImage, colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
...
void createDepthResources(struct application *app)
{
	...
	createImage(app,app->swapChainExtent.width, app->swapChainExtent.height, 1, app->msaaSamples, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &app->depthImage, &app->depthImageMemory);
	app->depthImageView = createImageView(app,app->depthImage, depthFormat,VK_IMAGE_ASPECT_DEPTH_BIT, 1);
}
